...
export class AdminController {
  ...
  async getDashboardStats(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      // Acil stabilite: admin dashboard artik canli Mikro sorgusu beklemez.
      // Mikro sorgulari stok/price sync sirasinda 120sn timeout'a takilabiliyor ve
      // ana paneli donmus gibi gosteriyordu. Buradaki ozetler lokal B2B verisinden
      // hesaplanir; Mikro agir raporlar kendi ekranlarinda kalir.
      {
        ...
        const orderScopeWhere = hasSectorScope ? { user: { sectorCode: { in: sectorCodes } } } : {};
        const quoteScopeWhere = hasSectorScope ? { customer: { sectorCode: { in: sectorCodes } } } : {};
        const periodCreatedAt = {
          gte: periodRange.start,
          lt: new Date(periodRange.end.getTime() + 24 * 60 * 60 * 1000),
        };
        const summarizeOrders = (where: any) => Promise.all([
          prisma.order.count({ where }),
          prisma.order.aggregate({ where, _sum: { totalAmount: true } }),
        ]).then(([count, amount]) => ({ count, amount: Number(amount._sum.totalAmount || 0) }));
        const summarizeQuotes = (where: any) => Promise.all([
          prisma.quote.count({ where }),
          prisma.quote.aggregate({ where, _sum: { grandTotal: true } }),
        ]).then(([count, amount]) => ({ count, amount: Number(amount._sum.grandTotal || 0) }));
        ...
        const [
          ...
          summarizeOrders({ status: 'APPROVED', approvedAt: periodCreatedAt, ...orderScopeWhere }),
          summarizeOrders({ createdAt: periodCreatedAt, ...orderScopeWhere }),
          summarizeQuotes({ createdAt: periodCreatedAt, ...quoteScopeWhere }),
        ]);
        ...
        const payload = {
          ...
          summary: {
            ...
            orders: { count: localOrderSummary.count, amount: localOrderSummary.amount },
            quotes: { count: localQuoteSummary.count, amount: localQuoteSummary.amount },
          },
          source: 'LOCAL_B2B',
        };
