/**  * Quote (Teklif) Service  *  * Teklif oluÅŸturma, admin onayÄ± ve Mikro'ya yazma iÅŸlemleri.  */ import { prisma } from "../utils/prisma";
...
const isValidCloseReason = (reason: string) =>
  QUOTE_LINE_CLOSE_REASONS.includes(reason);
...
const buildQuoteItemDiff = (
  ...
  afterItems.forEach((item) => {
    ...
    candidates.forEach((candidate, index) => {
      const score =
        Math.abs(candidate.unitPrice - snapshot.unitPrice) +
        Math.abs(candidate.quantity - snapshot.quantity);
      if (score < bestScore) {
        bestScore = score;
        bestIndex = index;
      }
    ...
    if (matched.lineDescription !== snapshot.lineDescription) {
      changes.lineDescription = {
        from: matched.lineDescription,
        to: snapshot.lineDescription,
      };
    }
...
class QuoteService {
  ...
  private async getLastQuotesForProducts(
    ...
    if (!customerId || codes.length == 0 || limit <= 0)
      return new Map<string, LastQuoteSnapshot[]>();
    ...
    const items = await prisma.quoteItem.findMany({
      ...
      orderBy: [{ quote: { createdAt: "desc" } }, { createdAt: "desc" }],
      take: takeCount,
      select: {
        productCode: true,
        unitPrice: true,
    ...
    for (const item of items) {
      ...
      if (mikroNumber) {
        ...
        if (parsed) {
          missingBelgePairs.set(
            `${parsed.evrakSeri}-${parsed.evrakSira}`,
            parsed,
          );
        }
      }
    ...
    if (missingBelgePairs.size > 0) {
      try {
        belgeNoMap = await mikroService.getQuoteBelgeNos(
          Array.from(missingBelgePairs.values()),
        );
      } catch (error) {
    ...
    for (const item of items) {
      ...
      if (mikroNumber) {
        ...
        if (parsed) {
          const mikroBelge =
            belgeNoMap.get(`${parsed.evrakSeri}-${parsed.evrakSira}`) || null;
          if (mikroBelge) {
      ...
      list.push({
        quoteDate: item.quote?.createdAt
          ? item.quote.createdAt.toISOString()
    ...
    if (mikroCariCode) {
      const needCodes = codes.filter(
        (code) => (map.get(code)?.length || 0) < limit,
      );
      if (needCodes.length) {
        try {
          ...
          for (const quote of mikroQuotes) {
            ...
            const entry: LastQuoteSnapshot = {
              quoteDate: quote.quoteDate
                ? new Date(quote.quoteDate).toISOString()
                : new Date().toISOString(),
          ...
          console.error("Mikro quote history fetch failed", {
            customerId,
            error,
          });
        }
    ...
    const sanitized = input
      .map((rule) => {
        ...
        const color =
          ...
          ["green", "yellow", "blue", "red", "slate"].includes(entry.color)
            ? entry.color
            : "green";
        const threshold = Number.isFinite(Number(entry.threshold))
  ...
  async getPreferences(userId: string) {
    ...
    return {
      ...
      whatsappTemplate:
        user?.quoteWhatsappTemplate || DEFAULT_WHATSAPP_TEMPLATE,
  ...
  async updatePreferences(
    ...
    if (data.lastSalesCount !== undefined) {
      updateData.quoteLastSalesCount = Math.max(
        1,
        Math.min(10, data.lastSalesCount),
      );
    }
    ...
    if (data.columnWidths !== undefined) {
      updateData.quoteColumnWidths = this.sanitizeColumnWidths(
        data.columnWidths,
      );
    }
    if (data.poolSort !== undefined) {
      updateData.quotePoolSort =
        data.poolSort && VALID_POOL_SORTS.has(data.poolSort)
          ? data.poolSort
          : "default";
    }
  ...
  async getCustomerLastQuoteItems(
    ...
    const map = await this.getLastQuotesForProducts(
      customerId,
      productCodes,
      safeLimit,
      excludeQuoteId,
  ...
  async createQuote(input: CreateQuoteInput, createdById: string) {
    ...
    const preparedItems = items.map((item, index) => {
      const quantity = Math.max(0.000001, safeNumber(item.quantity, 1));
      ...
      const unit2 = isManualLine
        ? item.unit2?.trim() || null
        : item.unit2?.trim() || (product as any).unit2 || null;
      const unit2FactorValue = safeNumber(
        item.unit2Factor ?? (product as any)?.unit2Factor,
        0,
      );
      const unit2Factor = unit2 && unit2FactorValue !== 0 ? unit2FactorValue : null;
      const selectedUnit = (item.selectedUnit?.trim() || unit).slice(0, 50);
      ...
      if (isManualLine) {
        ...
        if (!allowedCodes.includes(productCode)) {
          throw new Error(
            `Manual line product code must be one of: ${allowedCodes.join(", ")}`,
          );
        }
      }
      ...
      if (priceSource === "PRICE_LIST") {
        ...
        if (!listPrice) {
          throw new Error(
            `Selected price list has no price for ${productCode}`,
          );
        }
        unitPrice = listPrice;
        priceListNo = listNo;
      ...
      return {
        ...
        isBlocked,
        blockedReason,
        sourceSaleDate: item.lastSale?.saleDate
          ? new Date(item.lastSale.saleDate)
          : null,
        ...
        manualImageUrl: isManualLine
          ? item.manualImageUrl?.trim() || null
          : null,
        lineOrder: index + 1,
      };
    });
    ...
    const createItems = preparedItems.map((item) => {
      ...
      return productId
        ? { ...rest, product: { connect: { id: productId } } }
        : rest;
    });
  ...
  async updateQuote(
    ...
    const existing = await prisma.quote.findUnique({
      ...
      include: {
        items: { orderBy: { lineOrder: "asc" } },
        customer: {
          select: { id: true, mikroCariCode: true, paymentPlanNo: true },
        },
      },
    });
    if (!existing) {
    ...
    const resolvedResponsibleCode = (
      responsibleCode ||
      existing.responsibleCode ||
      ""
    ).trim();
    ...
    const updatedQuote = await prisma.$transaction(async (tx) => {
      ...
      await tx.quoteItem.createMany({
        data: preparedItems.map((item) => ({ quoteId, ...item })),
      });
  ...
  async getQuoteLineItems(params: {
    ...
    if (searchTerm) {
      where.OR = [
        ...
        {
          quote: { mikroNumber: { contains: searchTerm, mode: "insensitive" } },
        },
        {
        ...
        },
        {
          quote: {
            customer: {
              mikroCariCode: { contains: searchTerm, mode: "insensitive" },
            },
          },
        },
      ];
    ...
    if (Object.keys(dateFilter).length > 0) {
      if (normalizedStatus === "OPEN") {
        ...
        where.AND = [
          ...
          {
            OR: [
              { status: "OPEN", createdAt: dateFilter },
              { status: { not: "OPEN" }, statusUpdatedAt: dateFilter },
            ],
          },
        ];
      }
    ...
    const enriched = items.map((item) => {
      ...
      const baseDate =
        statusValue === "OPEN"
          ? item.createdAt || item.quote?.createdAt || item.statusUpdatedAt
          : item.statusUpdatedAt ||
            item.closedAt ||
            item.convertedAt ||
            item.createdAt ||
            item.quote?.createdAt;
      const baseTime = baseDate ? new Date(baseDate).getTime() : now.getTime();
      const waitingDays = Number.isFinite(baseTime)
        ? Math.max(0, Math.floor((now.getTime() - baseTime) / 86400000))
        : 0;
  ...
  async closeQuoteItems(input: {
    ...
    const grouped = new Map<
      string,
      {
        parsed: { evrakSeri: string; evrakSira: number };
    ...
    const historyEntries = Array.from(
      new Set(openItems.map((item) => item.quoteId)),
    ).map((quoteId) => ({
      quoteId,
      action: "UPDATED" as const,
      actorId: input.adminUserId,
      summary: "Teklif satirlari kapatildi",
      payload: { itemIds: openItems.map((item) => item.id) },
    }));
  ...
  async reopenQuoteItems(input: { itemIds: string[]; adminUserId: string }) {
    ...
    await prisma.$transaction(updates);
    const historyEntries = Array.from(
      new Set(closedItems.map((item) => item.quoteId)),
    ).map((quoteId) => ({
      quoteId,
      action: "UPDATED" as const,
      actorId: input.adminUserId,
      summary: "Teklif satirlari acildi",
      payload: { itemIds: closedItems.map((item) => item.id) },
    }));
    if (historyEntries.length > 0) {
      await prisma.quoteHistory.createMany({ data: historyEntries });
    }
  ...
  async syncQuoteFromMikro(quoteId: string) {
    ...
    const normalizedLines = mikroLines.map((line) => {
      ...
      const vatRate = mikroService.convertVatCodeToRate(
        safeNumber(line.vatCode, 0),
      );
    ...
    for (const [index, line] of normalizedLines.entries()) {
      ...
      if (!match) {
        match = quote.items.find(
          (item) =>
            !usedIds.has(item.id) && item.productCode === line.productCode,
        );
      }
      const unit = product?.unit || match?.unit || "ADET";
      const lineIsClosed = Boolean((line as any).isClosed);
      const fallbackStatusDate = line.lastUpdatedAt
        ? new Date(line.lastUpdatedAt)
        : new Date();
      const nextStatus =
      ...
      const nextStatusUpdatedAt =
        match && match.status === nextStatus
          ...
          : fallbackStatusDate;
      const nextClosedReason =
        nextStatus === "CLOSED"
          ? line.closeReason || match?.closedReason || null
          : null;
      const nextClosedAt =
        nextStatus === "CLOSED"
          ? match?.status === "CLOSED" && match.closedAt
            ? match.closedAt
            : fallbackStatusDate
          : null;
      ...
      const baseData = {
        ...
        productName,
        unit,
        unit2: product?.unit2 || match?.unit2 || null,
        unit2Factor: product?.unit2Factor || match?.unit2Factor || null,
        selectedUnit: match?.selectedUnit || unit,
    ...
    const totalVat = normalizedLines.reduce(
      (sum, line) => sum + line.lineTotal * (line.vatRate || 0),
      0,
    );
    const grandTotal = totalAmount + totalVat;
    ...
    if (
      nextValidityDate &&
      this.toDateKey(nextValidityDate) !== this.toDateKey(quote.validityDate)
    ) {
      changed = true;
    }
    if (!changed) {
    ...
    try {
      ...
      if (hasMikroOrder && !quote.convertedAt) {
        conversionUpdated = await prisma.quote.update({
          where: { id: quoteId },
          data: { convertedAt: new Date(), convertedSource: "MIKRO" },
          include: { items: { orderBy: { lineOrder: "asc" } } },
        });
        await prisma.quoteHistory.create({
          data: {
  ...
  async convertQuoteToOrder(
    ...
    const resolvedDocumentNo = String(
      documentNoInput ?? quote.documentNo ?? "",
    ).trim();
    const documentNo = resolvedDocumentNo || undefined;
    const documentNoChanged =
      resolvedDocumentNo !== String(quote.documentNo || "").trim();
    const selectedSet = new Set((selectedItemIds || []).filter(Boolean));
    const updateMap = new Map<
      string,
      { quantity?: number; responsibilityCenter?: string; reserveQty?: number }
    >();
    ...
    const normalizedItems = quote.items.map((item) => {
      ...
      const quantity =
        Number.isFinite(resolvedQuantity) && resolvedQuantity > 0
          ? resolvedQuantity
          : item.quantity;
      const resolvedReserveQty = Number(update?.reserveQty);
      const reserveQty =
        Number.isFinite(resolvedReserveQty) && resolvedReserveQty > 0
          ? Math.min(resolvedReserveQty, quantity)
          : 0;
      return {
        ...item,
        quantity,
    ...
    const unselectedItems = normalizedItems.filter(
      (item) => !selectedSet.has(item.id),
    );
    const openUnselectedItems = unselectedItems.filter(
      (item) => resolveStatus(item) === "OPEN",
    );
    ...
    const itemDiff = buildQuoteItemDiff(
      quote.items as any,
    ...
    if (hasDiff) {
      const totals = normalizedItems.reduce(
        (acc, item) => {
          ...
          const vatAmount = item.vatZeroed
            ? 0
            : lineTotal * (item.vatRate || 0);
    ...
    const guidByProduct = new Map<
      string,
      Array<{ guid: string; unitPrice: number; quantity: number }>
    >();
    ...
    const mappedItems = selectedItems.map((item) => {
      ...
      return {
        ...
        reserveQty: Math.max(Number((item as any).reserveQty || 0), 0),
        quoteLineGuid: guid,
      };
    });
    const invoicedItems = mappedItems.filter(
      (item) => item.priceType === "INVOICED",
    );
    ...
    if (invoicedItems.length > 0) {
      ...
      invoicedOrderId = await mikroService.writeOrder({
        ...
        evrakSira: Number.isFinite(Number(invoicedSira))
          ? Number(invoicedSira)
          : undefined,
        warehouseNo: warehouseValue,
        paymentPlanNo: quote.customer?.paymentPlanNo ?? undefined,
      });
      if (invoicedOrderId) {
    ...
    if (whiteItems.length > 0) {
      ...
      whiteOrderId = await mikroService.writeOrder({
        ...
        evrakSira: Number.isFinite(Number(whiteSira))
          ? Number(whiteSira)
          : undefined,
        warehouseNo: warehouseValue,
        paymentPlanNo: quote.customer?.paymentPlanNo ?? undefined,
        ...
        items: {
          create: selectedItems.map((item) => ({
            ...
            responsibilityCenter:
              (item as any).responsibilityCenter || undefined,
            status: "APPROVED",
        ...
        statusUpdates.push(
          prisma.quoteItem.update({
            ...
            data: {
              status: "CLOSED",
              closedReason:
                normalizeCloseReason(closeReasonsMap[item.id]) || null,
              closedAt: statusNow,
              statusUpdatedAt: statusNow,
            },
          }),
