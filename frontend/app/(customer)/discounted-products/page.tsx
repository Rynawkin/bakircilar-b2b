...
import { ChevronRight, Tag, Search, ArrowDownUp, X, SlidersHorizontal, Warehouse } from 'lucide-react';
...
export default function DiscountedProductsPage() {
  ...
  // Per-mode varsayilan sort = "En cok indirim". 'discount-desc' yerel; digerleri shared.
  const [sortBy, setSortBy] = useState<'discount-desc' | FilterState['sortBy']>('discount-desc');
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (sortBy !== 'discount-desc') count += 1;
      ...
      <div className={CONTAINER}>
        ...
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            <span className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl bg-[#ecfdf5] text-[#047857]">
          ...
          <div className="min-w-0">
            ...
            <div className="sticky top-[118px] z-20 mb-3.5 flex flex-wrap items-center gap-3 rounded-xl border border-[var(--line)] bg-white px-3.5 py-2.5 shadow-[0_1px_2px_rgba(20,34,59,.04)]">
              ...
              <label className="flex h-[38px] min-w-0 items-center gap-2 rounded-[9px] border border-[#e3e8f0] bg-white px-3">
                ...
                >
                  <option value="discount-desc">En çok indirim</option>
              ...
              <div className="flex flex-col items-center justify-center rounded-[14px] border border-[var(--line)] bg-white px-5 py-14 text-center">
                <span className="mb-3.5 flex h-14 w-14 items-center justify-center rounded-[14px] bg-[#ecfdf5] text-[#047857]">
                  {search || selectedCategory || warehouse || brandCodes.length > 0 ? (
                    ...
                    <Tag className="h-[26px] w-[26px]" strokeWidth={1.7} />
                  )}
                </span>
                <h2 className="mb-1.5 text-[17px] font-semibold text-[#14223b]">
                  {hasAnyActive ? 'Ürün bulunamadı' : 'İndirimli ürün bulunamadı'}
                </h2>
                <p className="mb-4 max-w-[360px] text-[13px] leading-relaxed text-[#8b97ac]">
                  {hasAnyActive
                    ? 'Seçtiğiniz kategori, marka ve filtre kombinasyonunda indirimli ürün yok. Filtreleri gevşetip tekrar deneyin.'
