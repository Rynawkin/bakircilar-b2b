...
const statusMeta: Record<PaymentStatus, { label: string; className: string }> = {
  CREATED: { label: 'Hazırlanıyor', className: 'bg-gray-100 text-gray-700' },
  PENDING: { label: 'Ödeme bekleniyor', className: 'bg-amber-50 text-amber-700' },
  SUCCEEDED: { label: 'Başarılı', className: 'bg-emerald-50 text-emerald-700' },
  FAILED: { label: 'Başarısız', className: 'bg-red-50 text-red-700' },
  EXPIRED: { label: 'Süresi doldu', className: 'bg-gray-100 text-gray-600' },
...
const amountTypeLabel: Record<PaymentAmountType, string> = {
  ...
  PAST_DUE: 'Vadesi geçen',
  CUSTOM: 'Özel tutar',
  EARLY_PAYMENT: 'Erken ödeme (indirimli)',
};
...
export default function CustomerPaymentsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (requestId !== loadRequestSequence.current) return;
      const message = error?.response?.data?.error || 'Ödeme bilgileri yüklenemedi. Lütfen tekrar deneyin.';
  ...
  useEffect(() => {
    ...
    if (!result) return;
    if (result === 'success') toast.success('Ödemeniz banka tarafından başarılı olarak doğrulandı.');
    else if (result === 'failure') toast.error('Ödeme tamamlanamadı. Kartınızdan tahsilat yapılmadıysa tekrar deneyebilirsiniz.');
    else if (result === 'review') toast('Ödeme sonucu banka ile kontrol ediliyor.', { icon: 'i' });
    else toast('Banka sonucu doğrulanıyor. Durum birazdan güncellenecek.', { icon: 'i' });
    if (paymentId) {
  ...
  const downloadReceipt = async (payment: PaymentAttempt) => {
    ...
    try {
      ...
      toast.error('Dekont oluşturulamadı. Lütfen tekrar deneyin.');
    } finally {
      setReceiptId(null);
    }
  };
  ...
  const createPayment = async () => {
    if (!summary?.eligibility.canCreate) {
      toast.error(summary?.eligibility.reason === 'BALANCE_STALE'
        ? 'Cari bakiye verisi güncel değil. Yeni rapor yüklendikten sonra tekrar deneyin.'
        : 'Online ödeme şu anda kullanılamıyor.');
      return;
    }
    if (!Number.isFinite(selectedAmount) || selectedAmount < summary.limits.min) {
      toast.error(`Ödeme tutarı en az ${formatCurrency(summary.limits.min)} olmalı.`);
      return;
    }
    if (selectedAmount > summary.availability.total) {
      toast.error('Ödeme tutarı ödenebilir bakiyeyi aşamaz.');
      return;
    }
    setCreating(true);
    try {
      ...
      if (!payment.paymentLinkUrl) throw new Error(payment.bankMessage || 'Banka ödeme bağlantısı oluşturmadı.');
      window.location.assign(payment.paymentLinkUrl);
    } catch (error: any) {
      const message = error?.response?.data?.error || error?.message || 'Ödeme bağlantısı oluşturulamadı.';
  ...
  const refreshPayment = async (id: string) => {
    ...
    try {
      ...
      await load();
      toast.success('Ödeme durumu bankadan kontrol edildi.');
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-[var(--surface-0)]">
      <div className="mx-auto w-full max-w-[1200px] px-3 py-5 sm:px-5 sm:py-7 lg:px-6">
        <nav className="mb-4 flex items-center gap-1.5 text-xs text-[var(--ink-3)]">
          ...
          <span>/</span>
          <span className="font-medium text-[var(--ink-2)]">Online Ödeme</span>
        </nav>
        ...
        <header className="mb-6 flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h1 className="text-[24px] font-bold text-[var(--ink-1)]">Online Ödeme</h1>
              <p className="mt-0.5 text-[13px] text-[var(--ink-3)]">Ziraat Bankası güvenli ödeme sayfası ile cari bakiye ödemesi</p>
            </div>
          </div>
          <Link href="/invoices" className="inline-flex h-10 items-center gap-2 rounded-lg border border-[var(--line)] bg-white px-3.5 text-[13px] font-semibold text-[var(--ink-2)] hover:bg-gray-50">
            <FileText className="h-4 w-4" /> Faturalarım
          </Link>
        </header>

        {loadError && (
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-900" role="alert">
            <div className="flex min-w-0 items-start gap-2.5">
              <AlertCircle className="mt-0.5 h-5 w-5 flex-none" />
              <div>
                <strong>Ödeme bilgileri yüklenemedi.</strong>
                <p className="mt-0.5 text-xs text-red-800">{loadError}</p>
              </div>
            </div>
            <button
              type="button"
              onClick={load}
              disabled={loading}
              className="inline-flex h-9 items-center gap-1.5 border border-red-300 bg-white px-3 text-xs font-semibold text-red-800 hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60"
            >
          ...
          <>
            <section className="mb-5 grid grid-cols-1 border-y border-[var(--line)] bg-white sm:grid-cols-3">
              ...
              <div className="border-t border-[var(--line)] px-5 py-4 sm:border-r sm:border-t-0">
                <p className="text-xs font-medium text-[var(--ink-3)]">Vadesi geçen</p>
              ...
              <div className="border-t border-[var(--line)] px-5 py-4 sm:border-t-0">
                <p className="text-xs font-medium text-[var(--ink-3)]">Ödenebilir tutar</p>
            ...
            <p className="-mt-3 mb-5 text-right text-[11px] text-[var(--ink-3)]">
              Bakiye verisi: {formatDateTime(summary.balance.updatedAt)} · Ödeme ve açık link rezervasyonları ödenebilir tutardan düşülmüştür.
            </p>
            ...
            {!summary.gateway.enabled && (
              <div className="mb-5 flex items-start gap-3 border-l-4 border-amber-500 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                <AlertCircle className="mt-0.5 h-5 w-5 flex-none" />
                <div><strong>Banka aktivasyonu bekleniyor.</strong> PayByLink API kullanıcı yetkisi tamamlandığında bu ekran otomatik olarak ödeme bağlantısı üretecek.</div>
              </div>
            )}
            ...
            {!summary.eligibility.balanceFresh && (
              <div className="mb-5 flex items-start gap-3 border-l-4 border-red-500 bg-red-50 px-4 py-3 text-sm text-red-900">
                <AlertCircle className="mt-0.5 h-5 w-5 flex-none" />
                <div><strong>Bakiye verisi güncel değil.</strong> Son bakiye {Math.round(summary.eligibility.balanceAgeHours)} saat önce yenilenmiş. Yeni vade/bakiye raporu yüklenene kadar ödeme bağlantısı oluşturulamaz.</div>
              </div>
            )}
            ...
            {(summary.availability.reserved > 0 || summary.availability.successfulUnreconciled > 0) && (
              <div className="mb-5 flex items-start gap-3 border-l-4 border-blue-500 bg-blue-50 px-4 py-3 text-sm text-blue-900">
                ...
                <div>
                  {summary.availability.reserved > 0 && <p>Açık ödeme bağlantılarında {formatCurrency(summary.availability.reserved)} bekliyor.</p>}
                  {summary.availability.successfulUnreconciled > 0 && <p>{formatCurrency(summary.availability.successfulUnreconciled)} başarılı ödeme muhasebe mutabakatı bekliyor.</p>}
                </div>
              </div>
            )}
            ...
            <section className="mb-7 grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
              <div className="border border-[var(--line)] bg-white p-4 sm:p-5">
                <h2 className="text-[16px] font-semibold text-[var(--ink-1)]">Ödenecek tutarı seçin</h2>
                <div className="mt-4 grid gap-2.5 sm:grid-cols-3">
                  {([
                    ['PAST_DUE', 'Vadesi geçen', summary.availability.pastDue],
                    ['TOTAL_BALANCE', 'Toplam bakiye', summary.availability.total],
                    ['CUSTOM', 'Özel tutar', null],
                    ...
                    >
                      ...
                      <span className="mt-1 block text-[15px] font-bold tabular-nums text-[#15356b]">{amount === null ? 'Tutarı siz girin' : formatCurrency(amount)}</span>
                    </button>
                  ))}
                </div>
                ...
                {amountType === 'CUSTOM' && (
                  <label className="mt-4 block">
                    <span className="mb-1.5 block text-xs font-semibold text-[var(--ink-2)]">Özel ödeme tutarı</span>
                ...
                <div className="mt-5 flex flex-col gap-3 border-t border-[var(--line)] pt-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="text-xs text-[var(--ink-3)]">Banka sayfasına aktarılacak tutar</p>
                  ...
                  >
                    ...
                    {creating ? 'Bağlantı oluşturuluyor' : 'Güvenli ödemeye geç'}
              ...
              <aside className="border border-[var(--line)] bg-[#14223b] p-5 text-white">
                ...
                <p className="mt-2 text-[13px] leading-5 text-slate-300">Kart ve 3D Secure işlemleri doğrudan Ziraat Bankası sayfasında tamamlanır. B2B yalnız banka tarafından kesin olarak doğrulanan sonucu kaydeder.</p>
                <div className="mt-4 border-t border-white/15 pt-4 text-xs leading-5 text-slate-300">
                  <p>Tek kullanımlık ödeme bağlantısı</p>
                  <p>Tutar banka ekranında değiştirilemez</p>
                  <p>Muhasebe onayı sonrası tahsilat makbuzu otomatik işlenir</p>
                </div>
              </aside>
            </section>
          </>
        ...
        <section className="overflow-hidden border border-[var(--line)] bg-white">
          <div className="flex items-center justify-between border-b border-[var(--line)] px-4 py-3.5 sm:px-5">
            <div>
              <h2 className="text-[15px] font-semibold text-[var(--ink-1)]">Ödeme geçmişi</h2>
              <p className="mt-0.5 text-xs text-[var(--ink-3)]">Banka bağlantıları ve muhasebe mutabakat durumu</p>
            </div>
            <button type="button" onClick={load} disabled={loading} className="inline-flex h-9 items-center gap-1.5 border border-[var(--line)] px-3 text-xs font-semibold text-[var(--ink-2)] hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-60">
              <RefreshCw className={`h-3.5 w-3.5 ${loading ? 'animate-spin' : ''}`} /> {loading ? 'Yenileniyor…' : 'Yenile'}
            </button>
          </div>

          {loadError && !summary ? (
            <div className="px-5 py-12 text-center text-sm text-red-700">Ödeme kayıtları yüklenemedi. Yukarıdaki “Tekrar dene” düğmesini kullanın.</div>
          ) : payments.length === 0 ? (
            <div className="px-5 py-12 text-center text-sm text-[var(--ink-3)]">Henüz online ödeme kaydı bulunmuyor.</div>
          ) : (
            <div className="overflow-x-auto">
              <div className="min-w-[760px]">
                <div className="grid grid-cols-[1.2fr_1fr_1fr_1fr_1.2fr] gap-3 bg-[var(--surface-0)] px-5 py-2.5 text-[10px] font-semibold uppercase text-[var(--ink-3)]">
                  <span>Tarih / Referans</span><span>Tür</span><span className="text-right">Tutar</span><span>Durum</span><span className="text-right">İşlem</span>
                </div>
                {payments.map((payment) => {
                  ...
                  return (
                    <div key={payment.id} className="grid grid-cols-[1.2fr_1fr_1fr_1fr_1.2fr] items-center gap-3 border-t border-[var(--line)] px-5 py-3 text-[12px]">
                      ...
                      <div><span className={`inline-flex rounded-full px-2 py-1 text-[10px] font-semibold ${meta.className}`}>{meta.label}</span>{payment.reconciledAt && <p className="mt-1 text-[10px] text-emerald-700">Muhasebeye işlendi</p>}</div>
