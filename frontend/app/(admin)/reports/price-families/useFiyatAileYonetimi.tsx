...
export const mapFamilyItemsToPool = (family: PriceFamily): PoolProduct[] =>
...
/**
 * Fiyat Aile Yonetimi ekraninin TUM is mantigi.
 * Klasik ve yeni gorunum bu hook'u tuketir; logic birebir korunmustur.
 * (Onceki PriceFamiliesPage component'inin `return (` oncesindeki her sey aynen tasinmistir.)
 */
export function useFiyatAileYonetimi() {
  const [families, setFamilies] = useState<PriceFamily[]>([]);
  ...
  const loadFamilies = async () => {
    ...
    try {
      const result = await adminApi.getPriceFamilies();
      setFamilies(result.data || []);
    } catch (error: any) {
      toast.error(error?.response?.data?.error || 'Fiyat aileleri alinamadi');
    } finally {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        ...
        mapped.forEach((product) => {
          if (!deduped.has(product.mikroCode)) deduped.set(product.mikroCode, product);
        });
  ...
  const startEdit = (family: PriceFamily) => {
    setMode('edit');
    setEditingId(family.id);
    setName(family.name || '');
    setCode(family.code || '');
    setNote(family.note || '');
    setActive(family.active !== false);
  ...
  const removeProduct = (productCode: string) => {
    setSelectedProducts((prev) => prev.filter((product) => product.mikroCode !== productCode.toUpperCase()));
  };

  const saveFamily = async () => {
    const familyName = name.trim();
    if (!familyName) {
      toast.error('Fiyat ailesi adi zorunlu');
      return;
    }
    ...
    try {
      ...
      if (mode === 'create') {
        await adminApi.createPriceFamily(payload);
        toast.success('Fiyat ailesi olusturuldu');
      } else if (editingId) {
        await adminApi.updatePriceFamily(editingId, payload);
        toast.success('Fiyat ailesi guncellendi');
      }
      await loadFamilies();
      resetForm();
    } catch (error: any) {
      toast.error(error?.response?.data?.error || 'Fiyat ailesi kaydedilemedi');
    } finally {
      setSaving(false);
    }
  };
  ...
  const deleteFamily = async (familyId: string) => {
    if (!window.confirm('Fiyat ailesi silinsin mi?')) return;
    setDeletingId(familyId);
    try {
      await adminApi.deletePriceFamily(familyId);
      setFamilies((prev) => prev.filter((item) => item.id !== familyId));
      if (editingId === familyId) resetForm();
      toast.success('Fiyat ailesi silindi');
    } catch (error: any) {
      toast.error(error?.response?.data?.error || 'Fiyat ailesi silinemedi');
    } finally {
      setDeletingId(null);
    }
  };
...
export default useFiyatAileYonetimi;
