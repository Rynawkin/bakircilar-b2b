...
import adminApi, { type BarterRadarResult, type BarterRadarRow } from '@/lib/api/admin';
import { formatCurrency, formatDateShort } from '@/lib/utils/format';

/**
 * Borc-Mal Takasi Radari (/reports/barter-radar).
 * Vadesi gecmis bakiyesi olan carilerin ana tedarikcisi oldugu / gecmiste
 * siparis verdigimiz urunlerdeki depo ihtiyacini eslestirir. SALT RAPOR.
...
// Accordion baslik: Cari | Vadesi Gecmis | Urun | Potansiyel | Sinirli Potansiyel | genislet
const HEAD_GRID = '2fr 1.1fr 70px 1.1fr 1.1fr 110px';
// Urun alt-tablosu: Urun | Rozetler | Merkez (min/stok/ihtiyac) | Topca (min/stok/ihtiyac) | Ihtiyac | Birim Maliyet | Tutar
const ITEM_GRID = '2fr 1.2fr 1.2fr 1.2fr 80px 1fr 1.1fr';
...
export default function BarterRadar() {
  ...
  const [data, setData] = useState<BarterRadarResult | null>(null);
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const response = await adminApi.getBarterRadar({ minPastDue });
  ...
  const toggleExpanded = (cariCode: string) => {
    setExpanded((prev) => {
  ...
  const renderDepotCell = (depot: { min: number; stock: number; need: number }) => (
    <span style={{ fontSize: 11, color: MUTED, lineHeight: 1.5 }}>
      ...
      <span style={{ display: 'block' }}>
        İhtiyaç{' '}
        <strong style={{ color: depot.need > 0 ? AMBER : FAINT }}>{formatQty(depot.need)}</strong>
      </span>
    </span>
  );
    ...
    <div style={{ maxWidth: 1280, margin: '0 auto', padding: 24 }}>
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 13, color: FAINT, marginTop: 5 }}>
            Vadesi geçmiş carilerin tedarik edebileceği ihtiyaç ürünlerini eşleştirir.
          </div>
        </div>
      ...
      <div style={{ ...cardStyle, padding: 16, marginBottom: 18 }}>
        ...
        <div style={{ fontSize: 11.5, color: FAINT, marginBottom: 12 }}>
          Bu tutarın üzerinde vadesi geçmiş bakiyesi olan cariler taranır.
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '220px auto', gap: 14, alignItems: 'end' }}>
          <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 18 }}>
        <div style={summaryCard}>
          <div style={{ fontSize: 11.5, color: FAINT }}>Eşleşen Cari</div>
          <div style={{ fontSize: 20, fontWeight: 600, color: INK, marginTop: 5 }}>
            {data ? nf.format(data.summary.matchedCustomerCount) : '-'}
          </div>
        </div>
        <div style={summaryCard}>
          <div style={{ fontSize: 11.5, color: FAINT }}>Toplam Potansiyel</div>
          <div style={{ fontSize: 20, fontWeight: 600, color: EMERALD, marginTop: 5 }}>
            {data ? formatCurrency(data.summary.totalPotential) : '-'}
          </div>
        </div>
        <div style={summaryCard}>
          <div style={{ fontSize: 11.5, color: FAINT }}>Toplam Vadesi Geçmiş</div>
          <div style={{ fontSize: 20, fontWeight: 600, color: RED, marginTop: 5 }}>
            {data ? formatCurrency(data.summary.totalPastDue) : '-'}
          </div>
        </div>
      </div>
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 11.5, color: FAINT, marginTop: 2 }}>
              {data
                ? `Min. ${formatCurrency(data.minPastDue)} vadesi geçmiş — ${nf.format(rows.length)} cari.`
                : 'Rapor bekleniyor.'}
            </div>
          </div>
        </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <div style={{ minWidth: 960 }}>
            ...
            {loading ? (
              ...
              rows.map((row: BarterRadarRow) => {
                const isExpanded = expanded.has(row.cariCode);
                return (
                  <div key={row.cariCode}>
                    ...
                    <div
                      ...
                      onClick={() => toggleExpanded(row.cariCode)}
                    >
