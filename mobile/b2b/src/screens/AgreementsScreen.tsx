...
export function AgreementsScreen() {
  ...
  const fetchAgreements = async () => {
    ...
    setError(null);
    try {
      const response = await customerApi.getProducts({ mode: 'agreements', search });
      setProducts(response.products || []);
    } catch (err: any) {
      setError(err?.response?.data?.error || 'Anlasmalar yuklenemedi.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const listHeader = useMemo(() => {
    return (
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Sabit fiyatli urunler listesi.</Text>
        <TextInput
          style={styles.search}
          ...
          onSubmitEditing={fetchAgreements}
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              ...
              {item.agreement && (
                <Text style={styles.code}>Min: {item.agreement.minQuantity}</Text>
              )}
              {renderPrices(item)}
